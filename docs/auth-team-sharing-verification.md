...
| Full CI-script suite | PASS ON LINUX | Python 3.13 in the `uv:python3.13-bookworm-slim` container: `uv run --no-project --with orjson --with packaging --with pytest --with pyyaml --with requests python -m pytest scripts/ci/ -q -p no:cacheprovider`: 158 passed, four existing configuration warnings, 7.16 seconds. The earlier Windows run had 153 passes and four unchanged release-planner failures caused by backslash paths passed to `git show` at `bundle_release_plan.py:444`. |
...
| Full frontend Jest coverage | EXECUTED; one baseline timeout | `TZ=UTC CI=true npm test -- --roots src --maxWorkers=2 --workerIdleMemoryLimit=1024MB --coverage --ci --watchAll=false --json --outputFile=test-results/agent2-casbin-jest.json`: 665/666 suites and 7,105/7,106 tests passed, one failed, zero skipped (688.799 seconds). The unchanged OAuth-modal save test exceeded its existing five-second timeout, as in the earlier full run. Its eight-test suite passed alone without changing the timeout (10.103 seconds); the full run remains failed. |
...
| Inherited core browser regression | HISTORICAL; aggregate not green | 194 collected: 128 passed, two failed, five flaky, 59 skipped, 30.6 minutes; report gate correctly failed. A follow-up with the deployment CI flag, fresh data, one worker and zero retries passed 46/47 (10.6 minutes), including all 40 deployment cases. Onboarding cleanup still failed on a preceding test's empty project, then passed alone on fresh data (one passed, zero retries, 2.1 minutes; report gate passed). Other failures concern shared test state/response timing; no core assertions or source were changed. The remaining existing platform/optional-feature skips are unexecuted coverage. |
...
| Eight registered Casbin browser journeys and report gate | PASS; final candidate rerun pending | Eight passed, zero failed/skipped/flaky/retried, 515.547 seconds. Startup identified `CasbinAuthorizationService` with enforcement/readiness/team/sharing enabled; the existing HTTP capability checks passed. Seven distinct authenticated users, auto-login disabled, durable audits, real persisted policy and the local provider were used. A read-only check of the resulting SQLite database found seven users, five shares, 32 `casbin_rule` rows and 270 audit rows. All four strict IBM scans had zero violations/ignored findings (7,064 passing checks; 96 potential violations, ten potential recommendations and four manual checks remain review items). |
