...
The authorization CI jobs install the locked backend extra and root test dependencies. The inherited backend unit job and Makefile unit/real-service targets also retain the extra; a dry run reproduced its removal by the old sync command. Focused contracts reproduced the missing extra, root dependencies and writer-path selection before correction. Browser acceptance requires a fresh startup record identifying `CasbinAuthorizationService`, enabled enforcement, ready team/sharing support, and the existing HTTP capabilities. Missing, native, disabled, unready, invalid-team, and duplicate startup records fail the gate. The first combined Casbin browser run passed; final combined-SHA acceptance remains pending. The native results below remain historical evidence.
...
| Full CI-script suite | HOSTED PASS on `76e861e996` | [Run 34180136393](https://github.com/waqoor/langflow/actions/runs/34180136393): 158 passed, four existing warnings, 2.78 seconds. The Linux container run also passed 158 tests. The earlier Windows run had 153 passes and four unchanged release-planner failures caused by backslash paths passed to `git show` at `bundle_release_plan.py:444`. |
...
| Frontend Jest coverage | HOSTED PASS on `76e861e996`; local timeout recorded | [Job 101918475356](https://github.com/waqoor/langflow/actions/runs/34180499489/job/101918475356), `make test_frontend_ci`: 566 suites / 6,667 tests passed, zero failures, 1,527.897 seconds. The existing target excludes the 100 axe suites / 439 tests, which all passed locally. The earlier full local run passed 7,105/7,106 tests; the unchanged OAuth-modal save test exceeded five seconds, then passed alone without changing that timeout. That local run remains failed. |
...
| Inherited core browser regression | HOSTED PASS on `76e861e996` | All 35 existing shards and [report gate 101924749529](https://github.com/waqoor/langflow/actions/runs/34180499489/job/101924749529) passed. JSON artifact `10039522806`: 161 passed, 12 skipped, zero failed/flaky/retried, 1,785.487 seconds. The configured skips remain unexecuted coverage. Earlier local aggregate failures are not relabelled as passes; no core assertions or source were changed. |
...
| Eight registered Casbin browser journeys and report gate | HOSTED PASS on `76e861e996`; final fixture rerun pending | [Execution](https://github.com/waqoor/langflow/actions/runs/34180499489/job/101918711576) and [report gate](https://github.com/waqoor/langflow/actions/runs/34180499489/job/101921958584): eight passed, zero failed/skipped/flaky/retried, one worker, 791.783 seconds. JSON artifact `10039189058` identifies enabled/ready `CasbinAuthorizationService`; existing HTTP capability checks, seven real authenticated users, disabled auto-login, persisted policy, durable audits and the local provider were used. Runtime: Casbin 1.43.0, Python 3.13.15, SQLite 3.53.1, Node 22.23.2, Playwright 1.60.0. Four strict IBM scans: zero violations/ignored findings, 6,513 passing checks; 82 potential violations, five potential recommendations and four manual checks remain review items. |
