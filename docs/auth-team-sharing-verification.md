...
| Full CI-script suite | HOSTED PASS on `7379170183` | [Run 34175932550](https://github.com/waqoor/langflow/actions/runs/34175932550): 158 passed, four existing warnings, 2.65 seconds. The Linux container run also passed 158 tests. The earlier Windows run had 153 passes and four unchanged release-planner failures caused by backslash paths passed to `git show` at `bundle_release_plan.py:444`. |
...
| Eight registered Casbin browser journeys and report gate | PASS on `7379170183`; correction rerun pending | Eight passed, zero failed/skipped/flaky/retried, 548.889 seconds. Startup identified `CasbinAuthorizationService` with enforcement/readiness/team/sharing enabled; the existing HTTP capability checks passed. Seven distinct authenticated users, auto-login disabled, durable audits, real persisted policy and the local provider were used. The resulting SQLite database contained seven users, 32 `casbin_rule` rows and 271 audit rows. All four strict IBM scans had zero violations/ignored findings (7,064 passing checks; 96 potential violations, ten potential recommendations and four manual checks remain review items). |
| Backend code handoff | PRELIMINARY; Agent 1 reported | 331 affected backend tests passed; 622 LFX service tests passed with five Windows-only skips. Scoped Ruff/pre-commit and a Linux secret scan passed. Scoped mypy with skipped imports has one diagnostic at HEAD and one current, zero added. Backend/LFX wheel and sdist builds and installed default/missing-dependency checks passed; the remaining runtime/package matrix is pending. These are working-diff results, not final-SHA acceptance. |
...
| Registered Casbin combined acceptance and package/runtime matrix | PENDING | Final combined-SHA browser/core/CI and the remaining Python 3.10/3.14/package checks are pending. Owner visibility still materializes owned-resource IDs; bounded large-owner listing performance is not established (§20.7). No native, skipped, historical, or collected-only result closes these requirements. |
