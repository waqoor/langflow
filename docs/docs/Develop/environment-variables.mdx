...
See [API keys and authentication](/api-keys-and-authentication) for identity and session settings, and [Authorization](authorization.mdx#configure-authorization-environment-variables) for native RBAC, team sharing, audit durability, and readiness settings. User/team collaboration requires `LANGFLOW_AUTO_LOGIN=false`; do not enable `LANGFLOW_AUTHZ_ENABLED` until migrations and `langflow authz teams-check` have succeeded.
