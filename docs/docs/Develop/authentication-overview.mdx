...
Authentication and authorization are configured independently. Authentication remains mandatory for identifying collaborators. This `waqoor/langflow` fork provides an optional, default-off Casbin authorization service for role-based access control (RBAC), selected through the existing service interface. The OSS pass-through default remains available without the optional dependency.
...
Set `LANGFLOW_AUTO_LOGIN=false` and use distinct authenticated accounts for team/resource collaboration; an auto-login superuser session cannot demonstrate user-to-user isolation. Install the fork's `langflow-base[authorization]` extra, register `CasbinAuthorizationService` in `lfx.toml`, and apply the authorization migration and readiness preflight before setting `LANGFLOW_AUTHZ_ENABLED=true`. The registered service evaluates a fresh policy snapshot derived from canonical role, team, membership, share, user, and resource state for each new admission.
