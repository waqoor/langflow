...
When `LANGFLOW_AUTHZ_ENABLED=true`, anonymous access also requires the registered authorization service to opt into public principals and allow the action. The native Langflow service accepts only the tenant/domain context already resolved by the server and never performs cross-tenant discovery. A replacement service must derive tenancy from trusted deployment configuration and server context; it must not treat the raw `Host` header as a tenant identity. A service that does not advertise public-principal support fails closed.
...
This fork restores **Admin settings** in the account menu at `/admin`. Its **Users** tab lets active superusers add users, edit usernames or passwords, change Active/Superuser status, and request account deletion. The **Teams** tab provides team creation and management on the same page; existing `/admin/teams` links open this tab. The page uses the existing [Users API](/api-users), which remains available for automation. Ordinary users cannot access the page or administer other accounts. User administration remains available when collaboration is explicitly disabled; team administration requires the collaboration service.

Account deletion retains the backend's resource-ownership and team-membership protections. An account that still owns resources must have those resources explicitly transferred or deleted first; the admin page does not cascade-delete them. Deactivate an account when access needs to end before resource disposition is complete.
...
1. Create a `.env` file with the following variables:
    ...
    `LANGFLOW_NEW_USER_IS_ACTIVE=False` keeps new accounts inactive until you activate them in **Admin settings** or with the Users API.
