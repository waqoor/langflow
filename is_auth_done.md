...
**Additional pre-PR gate remains unresolved:** the separate local Linux background real-service suite times out under the unchanged upstream watchdog. An isolated two-case pass does not clear that suite; the exact evidence is recorded in the verification ledger. Consequently, the final check is not an all-gates-pass result and does not support unconditional release or opening a PR on that premise.
